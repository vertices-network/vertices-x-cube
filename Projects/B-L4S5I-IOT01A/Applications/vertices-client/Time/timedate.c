 ...
 */
int
setRTCTimeDateFromNetwork(bool force_apply)
{
    int rc = TD_OK;
    int32_t ret = NET_OK;
    int32_t sock;
    ...
    /* +1 to be sure that the buffer is closed by a \0, so that it may be parsed by string commands. */
    char buffer[NET_BUF_SIZE + 1];
...
//                              strlen(lUserConfigPtr->tls_root_ca_cert) + 1);
        ret |= net_setsockopt(sock,
                              NET_SOL_SOCKET,
                              NET_SO_TLS_SERVER_NAME,
                              (void *) TIME_SOURCE_HTTP_HOST,
                              strlen(TIME_SOURCE_HTTP_HOST) + 1);
        ret |= net_setsockopt(sock,
                              NET_SOL_SOCKET,
                              NET_SO_TLS_SERVER_VERIFICATION,
                              (force_apply == true) ? (void *) &false_val : (void *) &true_val,
                              sizeof(bool));
#endif
    }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                int count = sscanf(dateStr,
                                   "%s %s %d %s %d %02d:%02d:%02d ",
                                   prefix,
                                   dow,
                                   &day,
                                   month,
