...
int
CaptureAndFlashPem(char *pem_name, char const *flash_addr);
 ...
 */
int
getInputString(char *inputString, size_t len)
{
    size_t currLen = 0;
    int c = 0;
 ...
 */
int
checkWiFiCredentials(const char **const ssid, const char **const psk, uint8_t *const security_mode)
{
...
updateWiFiCredentials(void)
{
    ...
    printf("\nWriting into Flash\r\n");
    ret =
        FLASH_update((uint32_t) &lUserConfigPtr->wifi_config, &wifi_config, sizeof(wifi_config_t));
...
CaptureAndFlashPem(char *pem_name, char const *flash_addr)
{
    ...
    ret = FLASH_update((uint32_t) flash_addr,
                       key_read_buffer,
 ...
 */
int
checkDefaultSeed(const char **const seed)
{
...
updateDefaultSeed(char *seed)
{
    ...
    ret =
        FLASH_update((uint32_t) &lUserConfigPtr->iota_config, &iota_config, sizeof(iota_config_t));
 ...
 */
int
checkTxConfig(const char **const full_node_url,
              const char **const tx_interval,
              const char **const temp_thresh)
{
...
updateTxConfig(void)
{
    ...
    ret = FLASH_update((uint32_t) &lUserConfigPtr->iota_tx_config,
                       &iota_tx_config,
                       sizeof(iota_tx_config_t));
